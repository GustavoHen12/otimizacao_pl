...
void le_entrada(int *quant_items, int *quant_pares_ord, int *capacidade_caminhao, int **pesos_itens, queue_items_t ***ordens){
  ...
  // Le pesos dos itens
  *pesos_itens = new_array(*quant_items+1);
  if(!*pesos_itens){
...
int numeroViagensUtilizadas(int *viagens_utilizadas, int tamanho){
  int tamanho_vet = 0;
  int *vet_viagem = new_array(tamanho);
  for(int i = 1; i < tamanho; i++){
    if((viagens_utilizadas[i]) > 0 && (vet_viagem[viagens_utilizadas[i]] == 0)){
      tamanho_vet++;
      vet_viagem[viagens_utilizadas[i]] = 1;
    }
  }
...
int branchBound(int item_para_alocar, int quant_items, int capacidade_caminhao, int *pesos_itens, int **mat_ordem, int **solucao_parcial, int *viagens) {
  // Base da recursao, chegou em uma folha
  if(item_para_alocar == quant_items+1){
    int quant_viagens = numeroViagensUtilizadas(viagens, quant_items+1);
  ...
  // Calcula parcial
  int valor_parcial = parcial(quant_items, capacidade_caminhao, pesos_itens, mat_ordem, solucao_parcial, viagens);
  ...
  if(valor_parcial < menor_numero_viagens){
    ...
    for(int i = 1; i <= quant_items; i++) {
      int ant_solucao_parcial = solucao_parcial[item_para_alocar][i];
      int ant_viagens = viagens[item_para_alocar];

      solucao_parcial[item_para_alocar][i] = 1;
      viagens[item_para_alocar] = i;
      
      if(viavel(viagens, quant_items, item_para_alocar, mat_ordem) == 1 && verificaViabilidade(i, quant_items, capacidade_caminhao, pesos_itens, solucao_parcial) == 1){        
        branchBound(item_para_alocar+1, quant_items, capacidade_caminhao, pesos_itens, mat_ordem, solucao_parcial, viagens);
      }
 
      solucao_parcial[item_para_alocar][i] = ant_solucao_parcial;
      viagens[item_para_alocar] = ant_viagens;
    }
  }
